...
val file_location = "/FileStore/shared_uploads/hemangsharma0801@gmail.com/diabetes.csv"
val file_type = "csv"
...
val diabetesDF = spark.read.format(file_type) 
...
diabetesDF.select("Pregnancies", "Glucose", "BloodPressure", "SkinThickness", "Insulin", "BMI", "DiabetesPedigreeFunction", "Age", "Outcome").describe().show()
...
diabetesDF.createOrReplaceTempView("DiabetesData");
...
val splits = diabetesDF.randomSplit(Array(0.7, 0.3))
val train = splits(0)
val test = splits(1)
...
val training = assembler.transform(train).select($"features", $"Outcome".alias("label"))
...
val testing = assembler.transform(test).select($"features", $"Outcome".alias("trueLabel"))
testing.show()
